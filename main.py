...
def edit_or_delete_student(username):
    st.subheader("✏️ ویرایش یا حذف دانش‌آموز")

    df = pd.read_sql_query("SELECT id, نام_دانش‌آموز, کلاس FROM students WHERE آموزگار = ?", conn, params=(username,))
    if df.empty:
        st.info("هیچ دانش‌آموزی ثبت نشده است.")
        return

    selected_id = st.selectbox("انتخاب دانش‌آموز:", df["id"])
    student_row = df[df["id"] == selected_id].iloc[0]

    new_name = st.text_input("نام جدید دانش‌آموز", value=student_row["نام_دانش‌آموز"])
    new_class = st.text_input("کلاس جدید", value=student_row["کلاس"])

    col1, col2 = st.columns(2)
    with col1:
        if st.button("💾 ثبت تغییرات"):
            cursor.execute("UPDATE students SET نام_دانش‌آموز = ?, کلاس = ? WHERE id = ?",
                           (new_name, new_class, selected_id))
            conn.commit()
            st.success("✅ اطلاعات دانش‌آموز با موفقیت به‌روزرسانی شد.")
            st.experimental_rerun()

    with col2:
        if st.button("🗑 حذف دانش‌آموز"):
            cursor.execute("DELETE FROM students WHERE id = ?", (selected_id,))
            conn.commit()
            st.warning("❌ دانش‌آموز حذف شد.")
            st.experimental_rerun()
def show_class_statistics_panel(username):
    st.subheader("📊 آمار کلی کلاس")

    # انتخاب درس
    lessons_df = pd.read_sql_query("SELECT DISTINCT درس FROM scores WHERE آموزگار = ?", conn, params=(username,))
    if lessons_df.empty:
    ...
    # گرفتن میانگین نمرات دانش‌آموزها در درس انتخاب‌شده
    df = pd.read_sql_query("""
        SELECT نام_دانش‌آموز, AVG(نمره) as میانگین_دانش‌آموز
        FROM scores
    ...
    if df.empty:
        st.info("برای این درس هنوز نمره‌ای ثبت نشده.")
        return

    # میانگین کل کلاس
    class_avg = df["میانگین_دانش‌آموز"].mean()

    # دسته‌بندی وضعیت‌ها
    status_counts = {1: 0, 2: 0, 3: 0, 4: 0}
    for avg in df["میانگین_دانش‌آموز"]:
        status = وضعیت_نمره‌ای(avg, class_avg)
    ...
    for text in texts + autotexts:
        ...
        text.set_horizontalalignment("right")  # راست‌چین

    ax.set_title(f"📊 توزیع وضعیت نمرات درس {selected_lesson}", fontproperties="Arial", fontsize=14)
    st.pyplot(fig)

    st.markdown(f"📈 میانگین کل کلاس در درس {selected_lesson}: **{round(class_avg, 2)}**")
...
def show_teacher_statistics_by_admin(school):
    # فیلتر آموزگارهای مدرسه
    teacher_df = pd.read_sql_query("SELECT نام_کاربر FROM users WHERE نقش = 'آموزگار' AND مدرسه = ?", conn, params=(school,))
    if teacher_df.empty:
...
def show_class_ranking_panel(username, role):
    ...
    # اگر آموزگار هست، فقط دانش‌آموزهای خودش
    # اگر مدیر یا معاون هست، باید آموزگار انتخاب کنه
    ...
    # انتخاب درس برای رتبه‌بندی درس‌به‌درس
    lessons_df = pd.read_sql_query("SELECT DISTINCT درس FROM scores WHERE آموزگار = ?", conn, params=(selected_teacher,))
    if lessons_df.empty:
        st.info("هیچ نمره‌ای ثبت نشده است.")
        return
    ...
    # رتبه‌بندی کلی میانگین همه دروس
...
def show_student_panel(username):
    ...
    # پیدا کردن نام نمایشی دانش‌آموز
...
def download_student_report_direct(student_name):
    # مشابه تابع آموزگار ولی بدون انتخاب دستی
    student_info = pd.read_sql_query("SELECT * FROM students WHERE نام_دانش‌آموز = ?", conn, params=(student_name,)).iloc[0]
