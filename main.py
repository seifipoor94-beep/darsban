...
def main_dashboard(user_data):
    # 1. بخش دفاعی: رفع مشکل 'نقش کاربر نامشخص است'
    user = {}
    ...
    # 2. استخراج و **پاکسازی** اطلاعات (اضافه شدن .strip())
    role_raw = user.get("نقش", "").strip() # پاکسازی رشته نقش از فواصل اضافی
    username = user.get("نام_کاربر")
    role = role_raw # استفاده از متغیر role_raw برای بررسی شرایط

    # بررسی نهایی نقش
    if not role:
        st.error("❌ نقش کاربری برای حساب شما تعریف نشده است. (در پایگاه داده).")
        return
    ...
    if role == "مدیر":
        ...
        st.error(f"❌ نقش کاربری '{role}' تعریف نشده است یا با مقادیر مورد انتظار مطابقت ندارد. (نقش دریافتی: '{role}')")
        return

    # نمایش Selectbox در بالای صفحه اصلی (که در موبایل منو را جایگزین می‌کند)
    ...
    # ----------------------------------------------------
    # 💻 ناوبری دسکتاپ (نمایش در Sidebar)
    # ----------------------------------------------------
    with st.sidebar:
        st.title("منوی اصلی")
    ...
    # ----------------------------------------------------
    # 3. فراخوانی پنل‌ها بر اساس انتخاب
    # ----------------------------------------------------
    if selected_panel == "داشبورد مدیر مدرسه":
        # مطمئن شوید این توابع در فایل شما وجود دارند
