...
def main_dashboard(user_data):
    # 1. تضمین دریافت داده‌ها: اگر user_data لیست است، عنصر اول را انتخاب کن
    if isinstance(user_data, list) and user_data:
        user = user_data[0]
    elif isinstance(user_data, dict):
        user = user_data
    else:
        st.error("خطا: ساختار داده‌های کاربر نامعتبر است.")
        return
    ...
    # بررسی نهایی نقش
    if not role:
        st.error("❌ نقش کاربر در داده‌های دریافتی پیدا نشد.")
        return
    ...
    if role == "مدیر":
        ...
        # اگر نقش پیدا شد اما یکی از موارد بالا نبود
        st.error("❌ نقش کاربر نامشخص است.")
        return

    # استفاده از st.selectbox در بالای صفحه اصلی برای ناوبری
    ...
    # ----------------------------------------------------
    # فراخوانی پنل‌ها بر اساس انتخاب
    # ----------------------------------------------------
